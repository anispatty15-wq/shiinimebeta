  ...
  const syncProfile = useCallback(async (u: User) => {
    ...
    try {
      ...
      if (adminSnap.status === 'fulfilled' && adminDoc) {
        isAdmin = adminDoc.exists() || u.uid === ADMIN_UID || u.email?.toLowerCase() === ADMIN_EMAIL;
        ...
        try {
          ...
          isAdmin = directAdminCheck.exists() || u.uid === ADMIN_UID || u.email?.toLowerCase() === ADMIN_EMAIL;
        } catch (fallbackError) {
