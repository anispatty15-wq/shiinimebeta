...
export const DonghuaAPI = {
  getHome:     (page = 1) => safeCall((ax) => ax.get(`/anime/donghua/home/${page}`),      parseDonghuaList, FALLBACK_LIST),
  getOngoing:  (page = 1) => safeCall((ax) => ax.get(`/anime/donghua/ongoing/${page}`),   parseDonghuaList, FALLBACK_LIST),
  getCompleted:(page = 1) => safeCall((ax) => ax.get(`/anime/donghua/completed/${page}`), parseDonghuaList, FALLBACK_LIST),
  getLatest:   (page = 1) => safeCall((ax) => ax.get(`/anime/donghua/latest/${page}`),    parseDonghuaList, FALLBACK_LIST),
  ...
  getByLetter: (slug: string, page = 1) => safeCall((ax) => ax.get(`/anime/donghua/az-list/${encodeURIComponent(slug)}/${page}`), parseDonghuaList, FALLBACK_LIST),
  getBySeason: (year: string)           => safeCall((ax) => ax.get(`/anime/donghua/seasons/${year}`), parseDonghuaList, FALLBACK_LIST),
  search:      (q: string, page = 1)    => safeCall((ax) => ax.get(`/anime/donghua/search/${encodeURIComponent(q.trim())}/${page}`), parseDonghuaList, FALLBACK_LIST),
...
export const HentaiAPI = {
  getHome:        (page = 1)   => safeCall((ax) => ax.get('/anime/nekopoi/home', { params: { page } }),             parseMediaList, FALLBACK_LIST),
