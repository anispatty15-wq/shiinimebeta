...
function pathToType(p: string): ContentType {
  if (p.startsWith('/comic') || p.startsWith('/maid')) return 'comic';
...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-white/[0.07] bg-bg/80 backdrop-blur-2xl pt-safe shadow-[0_8px_30px_rgba(0,0,0,0.14)]">
      ...
      <div className="max-w-screen-xl mx-auto px-4 h-14 flex items-center gap-2 px-safe">
          ...
          {NAV_LINKS.map(({ href, label }) => {
            ...
            return (
              <Link
                ...
                href={href}
                className={clsx(
                  'px-3 py-1.5 rounded-xl text-[0.78rem] font-medium whitespace-nowrap transition-all duration-200',
                  active ? 'bg-cyan/[0.11] text-cyan shadow-[inset_0_0_0_1px_rgba(0,229,255,0.12)]' : 'text-secondary hover:text-primary hover:bg-white/[0.05]'
                )}
              >
                {label}
              </Link>
        ...
        {/* Home has a larger search field in the hero. */}
        {showHeaderSearch && (
        ...
        <div className="md:hidden flex items-center gap-1">
          ...
          {/* Home already has a prominent search field below the hero. */}
          {showHeaderSearch && (
            <button
