    ...
    <div className="fixed left-0 bottom-28 z-[54] flex flex-col items-start gap-2.5 select-none">
      ...
      {!wuwaHidden && (
        <div className="flex items-center">
          ...
          {/* Tab — icon only, arrow when open */}
          <button onClick={() => toggle('wuwa')}
            aria-label="Wuwa Tracker"
            className={clsx(
              'flex items-center justify-center w-10 h-10 rounded-r-xl',
              'border border-l-0 border-border shadow-lg transition-all duration-200',
              open === 'wuwa' ? 'bg-sky-500/20 border-sky-400/40' : 'bg-surface hover:bg-surface-2'
            )}>
            {open === 'wuwa' ? (
              <ChevronLeft className="w-4 h-4 text-sky-400" aria-hidden />
            ) : (
              /* eslint-disable-next-line @next/next/no-img-element */
              <img src="/wuwa.gif" alt="Wuwa"
      ...
      {!waHidden && (
        <div className="flex items-center">
          ...
          <button onClick={() => toggle('wa')}
            ...
            {open === 'wa' ? (
              <ChevronLeft className="w-4 h-4 text-[#25D366]" aria-hidden />
            ) : (
              <div className="w-7 h-7 rounded-lg bg-[#25D366] flex items-center justify-center shadow-sm">
