...
function extractServers(raw: Record<string, unknown>): Server[] {
  // Try every known field name the API might use
  const arr = getArr(
    raw,
    'servers', 'streamingLinks', 'streaming', 'links',
    'mirror', 'mirrors', 'players', 'sources', 'videos'
  );

  if (arr.length > 0) {
    return arr.flatMap((s) => {
      const item = s as Record<string, unknown>;
      // Some APIs nest servers inside quality groups: { quality:'720p', servers:[...] }
  ...
  // Fallback: maybe the episode data IS a direct URL string
  const directUrl = getStr(raw, 'url', 'link', 'src', 'file', 'iframe', 'embed', 'streamUrl', 'videoUrl');
  if (directUrl) {
    return [{ name: 'Server 1', url: directUrl, isIframe: true }];
  }
  return [];
}
...
function extractDownloads(raw: Record<string, unknown>): DlGroup[] {
  ...
  }).filter((g) => g.links.length > 0 || g.quality);
}
...
export default function AnimeEpisodePage() {
  ...
  const title        = ep ? getStr(ep, 'title', 'episodeTitle', 'name', 'episode') : '';
  const number       = ep ? getStr(ep, 'number', 'episode', 'ep', 'episodeNumber') : '';
  const poster       = ep ? getStr(ep, 'poster', 'image', 'thumbnail', 'cover') : '';
  const prevEpisode  = ep ? getStr(ep, 'prevEpisode', 'prevSlug', 'prev', 'previousEpisode', 'prevUrl') : '';
  const nextEpisode  = ep ? getStr(ep, 'nextEpisode', 'nextSlug', 'next', 'nextUrl') : '';
  const seriesSlug   = ep ? getStr(ep, 'seriesSlug', 'animeSlug', 'series', 'parent') : '';
  ...
  const { resumeState, attachRef } = useVideoProgressSaver(
    ep && slug
      ? { slug, seriesSlug, title, episodeTitle: title, poster, type: 'anime' }
      : null
  );
    ...
    <div className="max-w-screen-xl mx-auto">
      ...
      <div className="w-full aspect-video bg-black relative">
        {activeServer ? (
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 text-muted">
            ...
            <p className="text-sm">
              {servers.length === 0
                ? 'Server belum tersedia — coba pilih server di bawah'
      ...
      <div className="px-4 pt-4 pb-2 flex items-start justify-between gap-3">
        <div className="min-w-0">
          <h1 className="text-sm font-bold text-primary line-clamp-2">
            {title || slug?.replace(/-/g, ' ')}
          </h1>
          {number && (
            <p className="text-xs text-muted mt-0.5">Episode {number}</p>
          )}
        </div>
        <div className="flex gap-2 flex-shrink-0">
          {prevEpisode && (
            <button
              onClick={() => router.push(`/anime/episode/${prevEpisode}`)}
          ...
          {nextEpisode && (
            <button
              onClick={() => router.push(`/anime/episode/${nextEpisode}`)}
          ...
          {seriesSlug && (
            <button
              onClick={() => router.push(`/anime/${seriesSlug}`)}
      ...
      <div className="px-4 pb-4">
        <p className="text-xs font-semibold text-muted uppercase tracking-wide mb-2.5 flex items-center gap-1.5">
          ...
          {servers.length === 0 && (
            <span className="text-orange-400 font-normal normal-case ml-1">
              — tidak ada server dari API
            </span>
          )}
        </p>
        {servers.length > 0 ? (
          <div className="flex flex-wrap gap-2">
            {servers.map((srv, i) => (
              <button
                ...
                className={clsx(
                  'ep-pill',
                  activeServer?.url === srv.url && 'active'
                )}
              >
                {srv.name}
                {srv.quality ? ` · ${srv.quality}` : ''}
              </button>
            ))}
          </div>
        ) : (
          <p className="text-xs text-muted">
            API tidak mengembalikan link streaming untuk episode ini.
          </p>
        )}
      </div>
